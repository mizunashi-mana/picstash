...
import type { CreateImpressionInput } from '@/application/ports/recommendation-conversion-repository.js';
...
export function recommendationConversionRoutes(
  app: FastifyInstance,
  container: AppContainer,
): void {
  const conversionRepository = container.getRecommendationConversionRepository();
  ...
  app.post<{ Body: CreateImpressionsBody }>(
    ...
    async (request, reply) => {
      ...
      try {
        const conversions
          = await conversionRepository.createImpressions(validatedInputs);
  ...
  app.patch<{ Params: { id: string }; Body: RecordClickBody }>(
    ...
    async (request, reply) => {
      ...
      const existing = await conversionRepository.findById(id);
      if (existing === null) {
        return await reply.status(404).send({
          error: 'Not Found',
      ...
      try {
        const conversion = await conversionRepository.recordClick(id, {
  ...
  app.get<{ Querystring: GetStatsQuery }>(
    ...
    async (request, reply) => {
      ...
      const stats = await conversionRepository.getStats({ days });
