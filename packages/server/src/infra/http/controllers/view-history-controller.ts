...
import type { ViewHistoryRepository } from '@picstash/core';
...
export class ViewHistoryController {
  ...
  registerRoutes(app: FastifyInstance): void {
    ...
    app.post<{ Body: CreateViewHistoryBody }>('/api/view-history', async (request, reply) => {
      ...
      try {
        const viewHistory = await this.viewHistoryRepository.create({
    ...
    app.patch<{ Params: { id: string }; Body: UpdateViewHistoryBody }>(
      ...
      async (request, reply) => {
        ...
        const existing = await this.viewHistoryRepository.findById(id);
        if (existing === null) {
          return await reply.status(404).send({
            error: 'Not Found',
        ...
        const viewHistory = await this.viewHistoryRepository.updateDuration(id, { duration });
    ...
    app.get<{ Querystring: ListViewHistoryQuery }>('/api/view-history', async (request, reply) => {
      ...
      const viewHistory = await this.viewHistoryRepository.findRecentWithImages({ limit, offset });
      return await reply.send(viewHistory);
    });
    ...
    app.get<{ Params: { id: string } }>('/api/images/:id/view-stats', async (request, reply) => {
      const { id } = request.params;
      const stats = await this.viewHistoryRepository.getImageStats(id);
